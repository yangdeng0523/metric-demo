...
from models import (
    ...
    DownstreamModel, DownstreamApp, Dataset, AppDatasetGrant,
...
def seed_metadata(s):
    ...
    atomics = [
        AtomicMetric(code="order_count", name="下单次数", process_id=processes[0].id,
                     ...
                     owner="张涛", cert_level="QUALITY",
                     biz_definition="订单明细行数（COUNT 不去重），含全部订单状态",
        ...
        AtomicMetric(code="order_amount_sum", name="下单金额", process_id=processes[0].id,
                     ...
                     owner="张涛", cert_level="CERTIFIED",
                     biz_definition="订单商品金额合计（含税），以用户下单时刻金额为准",
        ...
        AtomicMetric(code="pay_amount_sum", name="支付金额", process_id=processes[1].id,
                     ...
                     data_type="DECIMAL", unit="元", status=STATUS_PUBLISHED,
                     owner="李雪", cert_level="QUALITY",
                     biz_definition="实际支付金额合计，仅统计支付成功流水（口径：支付成功）",
        ...
        AtomicMetric(code="pay_count", name="支付笔数", process_id=processes[1].id,
                     ...
                     data_type="BIGINT", unit="次", status=STATUS_PUBLISHED,
                     owner="李雪", cert_level="CERTIFIED",
                     biz_definition="支付成功流水笔数，按支付日期归属",
                     description="支付成功笔数"),
        AtomicMetric(code="refund_amount_sum", name="退款金额", process_id=processes[2].id,
                     agg_function="SUM", physical_field="refund_amount",
                     data_type="DECIMAL", unit="元", status=STATUS_PUBLISHED,
                     owner="王芳", cert_level="COMMON",
                     biz_definition="退款金额合计，含各退款原因，以退款发生日期归属",
                     description="退款金额合计"),
    ]
    s.add_all(atomics)
    s.flush()

    # 修饰词库（独立成库：时间周期 / 业务限定 / 统计粒度 可复用）
    modifiers = [
        # 时间周期
        MetricModifier(modifier_type="time_period", code="period_1d", name="最近1天",
                       config={"period": "1d"}, description="最近 1 个自然日"),
        MetricModifier(modifier_type="time_period", code="period_7d", name="最近7天",
                       config={"period": "7d"}, description="最近 7 个自然日"),
        MetricModifier(modifier_type="time_period", code="period_30d", name="最近30天",
                       config={"period": "30d"}, description="最近 30 个自然日"),
        MetricModifier(modifier_type="time_period", code="period_90d", name="最近90天",
                       config={"period": "90d"}, description="最近 90 个自然日"),
        MetricModifier(modifier_type="time_period", code="period_ytd", name="年初至今",
                       config={"period": "ytd"}, description="当年 1 月 1 日至今天"),
        # 业务限定
        MetricModifier(modifier_type="business_filter", code="filter_valid_order",
                       name="有效订单",
                       config={"filters": [{"field": "order_status", "op": "IN",
                                             "value": ["PAID", "SHIPPED"]}]},
                       description="仅统计已支付/已发货的有效订单"),
        MetricModifier(modifier_type="business_filter", code="filter_wechat_pay",
                       name="微信支付",
                       config={"filters": [{"field": "pay_channel", "op": "=",
                                             "value": "WECHAT"}]},
                       description="仅统计微信支付渠道"),
        # 统计粒度
        MetricModifier(modifier_type="granularity", code="gran_city", name="按城市",
                       config={"dim_codes": ["dim_city"]}, description="按城市维度统计"),
        MetricModifier(modifier_type="granularity", code="gran_category", name="按类目",
                       config={"dim_codes": ["dim_category"]}, description="按商品类目统计"),
        MetricModifier(modifier_type="granularity", code="gran_user", name="按用户",
                       config={"dim_codes": ["dim_user"]}, description="按用户维度统计"),
    ]
    s.add_all(modifiers)

    # 派生指标（需求文档 7.2.3）：引用修饰词库（可复用，改库即改口径）
    derived = [
        DerivedMetric(code="pay_amount_7d_city", name="最近7天各城市支付金额",
                      atomic_id=atomics[2].id, time_period="7d",
                      dim_codes=["dim_city"], filters=[],
                      modifier_codes=["period_7d", "gran_city"],
                      compare_type="yoy_mom",
                      owner="李雪", cert_level="CERTIFIED",
                      biz_definition="最近 7 天按城市汇总的支付金额，同比/环比自动派生（与去年同窗口、与前一 7 天窗口对比）",
        ...
        DerivedMetric(code="pay_count_7d_city", name="最近7天各城市支付笔数",
                      ...
                      compare_type="yoy_mom",
                      owner="李雪", cert_level="CERTIFIED",
                      biz_definition="最近 7 天按城市汇总的支付笔数，同比/环比自动派生",
        ...
        DerivedMetric(code="order_amount_30d_cat", name="最近30天各类目下单金额",
                      ...
                      filters=[{"field": "order_status", "op": "IN",
                                "value": ["PAID", "SHIPPED"]}],
                      modifier_codes=["period_30d", "filter_valid_order", "gran_category"],
                      compare_type="mom",
                      owner="张涛", cert_level="QUALITY",
                      biz_definition="最近 30 天按类目汇总、仅统计有效订单（PAID/SHIPPED）的下单金额，环比自动派生",
        ...
        DerivedMetric(code="refund_amount_7d_city", name="最近7天各城市退款金额",
                      ...
                      compare_type="none",
                      owner="王芳", cert_level="COMMON",
                      biz_definition="最近 7 天按城市汇总的退款金额（按退款日期归属）",
    ...
    composites = [
        CompositeMetric(code="avg_order_value", name="客单价",
                        ...
                        owner="李雪", cert_level="QUALITY",
                        biz_definition="平均每笔支付金额 = 最近 7 天支付金额 ÷ 支付笔数（按城市对齐）",
        ...
        CompositeMetric(code="refund_rate", name="退款率",
                        ...
                        owner="王芳", cert_level="CERTIFIED",
                        biz_definition="退款金额占支付金额比例，零值分母返回空",
    ...
    s.add_all([
        ...
        CodeRule(entity_type="modifier", pattern=r"^[a-z][a-z0-9_]*$", example="period_7d"),
    ])
