# 需求文档 vs 阿里云 DataMetric 功能对照分析

> 分析对象：《统一指标维度管理平台 Demo 需求文档》
> 对比基准：阿里云 DataMetric「规范定义 / 数仓规划」逻辑层能力
> 结论先行：**Demo 需求完整覆盖了 DataMetric 逻辑层的核心语义（主题域→业务过程→原子指标→派生指标→复合指标 + 维度 + 逻辑模型），实现方式与 DataMetric 同构；差异集中在 DataMetric 的「生产配套」能力（审批发布、版本、质量、调度）和「修饰词」是否独立成实体这两处。**

---

## 1. 需求功能清单 vs DataMetric 能力对照

| # | 需求功能（优先级） | DataMetric 对应能力 | 本 Demo 实现状态 | 差异 / 等价性说明 |
|---|---|---|---|---|
| 1 | 主题域管理（P0） | 业务板块 → 主题域（数据规划） | ✅ 完整 CRUD + 分页搜索 + 删除引用校验 | DataMetric 是「业务板块 > 主题域」两级；需求文档只取「主题域」一级，语义对齐 |
| 2 | 业务过程管理（P0） | 业务过程（事件型过程，关联事实表） | ✅ 完整 CRUD + 按主题域筛选 + 详情含原子指标列表 + 删除校验 | 与 DataMetric「业务过程」完全对应，含 physical_table 关联 |
| 3 | 原子指标管理（P0） | 原子指标（业务过程 + 聚合逻辑 + 物理字段） | ✅ CRUD + 状态流转(DRAFT/PUBLISHED/ARCHIVED) + 引用此指标的派生指标列表 + 删除校验(409) | 聚合函数 SUM/COUNT/AVG/MAX/MIN/COUNT_DISTINCT 与 DataMetric 一致 |
| 4 | 维度管理（P0） | 维度 + 维度属性（含主键属性） | ✅ 维度 CRUD + 属性增删改 + 删除校验 | `is_primary_key` 对齐 DataMetric 维度主键属性 |
...
| 7 | 逻辑模型映射（P1） | 逻辑表（维度/事实逻辑表）| ✅ 逻辑模型 CRUD + SINGLE/JOIN 类型 + JOIN 配置 + generated_sql | 对应 DataMetric 事实逻辑表（明细/汇总）+ JOIN 关联 |
| 8 | 血缘追溯（P2） | 血缘分析（影响分析 + 根因分析） | ✅ 全链路血缘：物理字段→原子→派生→复合 + 双向追溯 | 节点类型/边方向与 DataMetric 血缘一致，支持从任意节点展开 |
| 9 | 可视化看板（P2） | 指标结果图表展示 | ✅ 前端 Chart 渲染（水平条形图） | DataMetric 是报表/Analytics 层，Demo 用前端原生图表等价覆盖 |
...
| 需求文档表 | DataMetric 对应元数据 | 说明 |
|---|---|---|
| subject_domain | 业务板块 → 主题域 | DataMetric 两级（业务板块>主题域），需求取一级，编码/名称/排序一致 |
| business_process | 业务过程 | 相同概念，关联物理事实表 |
| atomic_metric | 原子指标 | 相同概念 + `status`（DataMetric 用「发布状态」） |
| dimension / dimension_attribute | 维度 / 维度属性 | 相同概念 |
| derived_metric | 派生指标（修饰词组合） | DataMetric 修饰词分四类：时间修饰词、其他修饰词（业务限定）、维度修饰词（统计粒度）、枚举修饰词；需求把「时间周期 + 统计维度 + 筛选条件」内嵌为三个字段 |
| composite_metric | 复合指标 | 表达式 + 引用指标列表，完全对齐 |
| logical_model | 逻辑表 | 需求演化为「物理表映射 + 可配置 JOIN」 |

> 一致性：9/9 张表在 DataMetric 中都有对应物，无凭空设计。
...
需求文档定义：**派生指标 = 原子指标 + 时间周期 + 统计粒度（维度）+ 业务限定（筛选条件）** —— 这正是 DataMetric 派生指标的完整定义。
...
## 7. 差距清单（DataMetric 提供、需求未覆盖 → Demo 不做）

| 差距项 | DataMetric 能力 | 是否影响需求达标 |
|---|---|---|
...
| 数据资产目录（专辑/文件夹） | 指标分类树、专辑收藏 | 用主题域二级结构替代（对应 DataMetric 主题域规划） |
| 智能（AI 建模/问答） | ChatBI、自动建模 | 超出 Demo 范围 |

## 8. 结论

1. **语义层面**：需求文档复现了 DataMetric 逻辑层的核心——"口径统一、复用高效、变更可控、血缘透明"四大核心价值全部对应落地；「派生指标 = 原子指标 + 修饰词」公式与 DataMetric 完全一致。
2. **实现层面**：Demo 与 DataMetric 的差别主要是**生产化能力**（审批、版本、质量、调度、权限），而非语义缺陷；作为轻量级 Demo（P0/P1/P2 全实现）是恰当的裁剪。
3. **可扩展性**：Schema 中 `derived_metric` 的三个修饰字段若后续要升级为 DataMetric 修饰词体系，只需拆成独立的修饰词表并加外键，无需改动 SQL 生成引擎核心——演进路径平滑。